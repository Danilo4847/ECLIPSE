...
public class MenuChamado {
	...
	public void apresentarMenuChamado(UsuarioVO usuarioAutenticado) {
		...
		while(opcao!=OPCAO_SAIR) {
			...
			case OPCAO_REALIZAR_CHAMADO:{
				this.cadastrarChamado(usuarioAutenticado);
				break;
			}
	...
	private void cadastrarChamado(UsuarioVO usuarioAutenticado) {
		ChamadoVO novoChamado = new ChamadoVO();
		novoChamado.setIdusuario(usuarioAutenticado.getIdusuario());
		
		System.out.println("\nDigite o titulo");
		novoChamado.setTitulo(teclado.nextLine());

		System.out.println("Digite a descrição");
		novoChamado.setDescricao(teclado.nextLine());

		novoChamado.setData(LocalDate.now());
		System.out.println("digite a data");

		if(this.validarCamposChamados(novoChamado)) {
			ChamadoController chamadoController = new ChamadoController();
			novoChamado = chamadoController.cadastrarChamadoController(novoChamado);

			if(novoChamado.getIdchamado()!=0) {
